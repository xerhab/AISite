...
username = 'svc_aisite'
password = 'aiSh4feegh!'
cnxn = pyodbc.connect('DRIVER={/opt/microsoft/msodbcsql18/lib64/libmsodbcsql-18.2.so.1.1};SERVER='+server+';DATABASE='+database+';UID='+username+';PWD='+ password+';sslmode=require;TrustServerCertificate=yes')

def same_auth(username, password):
    # cnxn = pyodbc.connect('DRIVER={/opt/microsoft/msodbcsql18/lib64/libmsodbcsql-18.2.so.1.1};SERVER='+server+';DATABASE='+database+';UID='+username+';PWD='+ password+';sslmode=require;TrustServerCertificate=yes')
    cursor = cnxn.cursor()
    cursor.execute("SELECT COUNT(*) FROM credentials WHERE username=? AND password=?", (username, password))
...
def logout_callback():
    global same_auth
    same_auth = False
    print("Logout button clicked, logout_callback function called")
...
def create_interface():
    ...
    with gr.Blocks(css=ui.css if not any((shared.args.chat, shared.args.cai_chat)) else ui.css+ui.chat_css, analytics_enabled=False, title=title) as shared.gradio['interface']:
        shared.gradio['Logout'] = gr.Button('Logout', elem_id="logout")
        shared.gradio['Logout'].click(logout_callback)            
        if shared.args.chat or shared.args.cai_chat:
            ...
            shared.gradio['Logout'].onclick(logout_callback)
    ...
    # Launch the interface with the retrieved credentials
    shared.gradio['interface'].queue()
    if shared.args.listen:
        shared.gradio['interface'].launch(prevent_thread_lock=True, share=shared.args.share, server_name='0.0.0.0', server_port=shared.args.listen_port, inbrowser=shared.args.auto_launch, auth=same_auth)
    else:
        shared.gradio['interface'].launch(prevent_thread_lock=True, share=shared.args.share, server_port=shared.args.listen_port, inbrowser=shared.args.auto_launch, auth=same_auth)
